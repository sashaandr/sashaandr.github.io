$(document).ready(function () {
	...
	const btn = $('.aside .filter__reboot');
	...
	/*$('.filter-item__head').click(function(){
		...
		$(this).parent().find('.checkbox-list').find('.checkbox').find('.checkbox__input').prop('checked', false);;
	...
	*/
    ...
    $(window).on('load resize', function() {
	    ...
	    const checkbox = $('.checkbox__input');
		const priceMin = $('#price-min');
		const priceMax = $('#price-max');

		$('.filter__reboot').on('click', function(){
			$( "#price" ).slider( "values", [ 500, 100000 ] );
			priceMin.val('500');
			priceMax.val('100000');
		...
		$('.filter-item__icon').on('click', function(){
			$(this).closest('.filter-item').find('.checkbox__input').prop('checked', false);
		});

		$('.filter-item_price .filter-item__icon').on('click', function(){
			priceMin.val('500');
			priceMax.val('100000');
			$( "#price" ).slider( "values", [ 500, 100000 ] );
		});
